...
- 対象コンポーネント: API Gateway (HTTP API), Lambda (App/Worker), RDS(Aurora Serverless), S3(Object Storage), SQS, SES, WAF, IAM
...
| INF-API-002 | API Gateway (HTTP API) | WAF適用 | WebACLIdが設定 |
| INF-API-003 | API Gateway (HTTP API) | レート制限 | StageのDefaultRouteSettingsにThrottle設定（UsagePlanは非対応） |
| INF-API-004 | Lambda App | VPC内 | VpcConfigが存在 |
| INF-API-005 | Lambda App | 追跡 | Tracing=Active |
| INF-API-006 | Lambda App | 同時実行制御 | ReservedConcurrentExecutionsが設定 |
| INF-API-007 | Lambda App | 環境分離 | 環境変数にSTAGE設定 |
| INF-API-008 | Lambda App | ログ保持 | LogGroup Retention >= 30日 |
| INF-API-009 | Lambda App | 最小権限 | IAM Policyが最小権限 |
...
| INF-OPS-005 | cdk-nag | AwsSolutionsChecks | エラー0件 |
